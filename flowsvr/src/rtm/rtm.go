...
var (
	...
	subTableKey            string = "subTable"
	updateSchedulePosKey   string = "updateSchedulePos"
	dealLongTimeProcessKey string = "dealLongTimeProcess"
)
...
func (p *TaskRuntime) UpdateSchedulePosProcessing() {
	// 1. 读取position表中的记录，映射为map[string]*db.TaskPos
...
func (p *TaskRuntime) subTableProcessing() {
	...
	// 2. 遍历切片，统计taskType_endPos表的记录总数count / 判断是否有id == count的记录
	for _, taskPos := range taskPosList {
		taskType := taskPos.TaskType
		...
		// 获取有没有id == count的记录
		_, err = db.TaskNsp.FindByIdAndTypeAndPos(db.DB, config.Conf.Task.TableMaxRows, taskType, endPos)
		if errors.Is(err, gorm.ErrRecordNotFound) {
			fmt.Println("没找到记录 id == ", config.Conf.Task.TableMaxRows)
		} else if err != nil {
			martlog.Errorf("find record error where id == %d %s", config.Conf.Task.TableMaxRows, err.Error())
			return
		} else {
			fmt.Println("找到了id == 最大行数的记录, 需要分表")
			// 找到了id == 最大行数的记录, 需要分表
			// 2.1 创建一个新的表task_type_endPos+1
			nextPos := db.TaskPosNsp.GetNextPos(endPos)
			err = db.TaskNsp.CreateTable(db.DB, taskType, nextPos)
...
func (p *TaskRuntime) dealTimeoutProcessingWithType(taskCfg *db.TaskScheduleCfg) {
	...
	taskList, err := db.TaskNsp.GetLongTimeProcessing(db.DB, taskCfg.TaskType,
		fmt.Sprintf("%d", taskPos.ScheduleBeginPos), fmt.Sprintf("%d", taskPos.ScheduleEndPos),
		maxProcessTime, 1000)
	if err != nil {
