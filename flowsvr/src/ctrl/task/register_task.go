...
// RegisterTasksHandler 注册任务：创建对应的任务信息表t_taskType_1，在位置信息表插入一条(beginPos, endPos)为(1, 1)的记录
...
func (p *RegisterTasksHandler) HandleProcess() error {
	...
	if err := db.TaskNsp.CreateTable(db.DB, p.Req.TaskType, "1"); err != nil {
		// 出现错误
		martlog.Errorf("RegisterTask newTable %v is exist, err is %v", fmt.Sprintf("%s_%s_%d", p.Req.TaskType, "task", 1), err)
