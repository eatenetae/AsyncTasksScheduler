...
func (p *Task) getTableName(taskType, pos string) string {
	return fmt.Sprintf("%s_%s_%s", taskType, p.TableName(), pos)
}
...
// FindByIdAndTypeAndPos 通过主键id查找记录
func (p *Task) FindByIdAndTypeAndPos(db *gorm.DB, id int, taskType, pos string) (*Task, error) {
	var data = &Task{}
	err := db.Table(p.getTableName(taskType, pos)).Where("id = ?", id).First(data).Error
	return data, err
}
...
func (p *Task) GetLongTimeProcessing(db *gorm.DB,
	taskType, startPos, endPos string, maxProcessTime int64, limit int) ([]*Task, error) {
	var Tasks = make([]*Task, 0)
	// 没分表的情况下
	err := db.Table(p.getTableName(taskType, startPos)).
	...
	// 分表的情况下
	if startPos != endPos {
		var newTasks = make([]*Task, 0)
		err := db.Table(p.getTableName(taskType, endPos)).
			Where("status = ?", TASK_STATUS_PROCESSING).
			Where("unix_timestamp(modify_time) + ? < ?", maxProcessTime, time.Now().Unix()).
			Limit(limit).
			Find(&newTasks).
